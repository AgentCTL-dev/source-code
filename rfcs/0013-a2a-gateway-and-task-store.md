...
> spellings it touches (`a2a.*`/`agentd://run|subagent`, the `agentd/…` `_meta` keys) are
...
```
   ...
   RFC 0008 ──────► │  A2A RELAY (DaemonSet, NODE-PINNED)│  write status │  SHARED DURABLE STORE         │
       ...
       to-point ⇒ relay on the node)▼
                    ┌───────────────────────────────────────────────────────────┐
                    │  conformant agent pod (reference impl: agentd) — NO network │
...
For the **live** methods the gateway is a frame re-enveloper. The mapping is mechanical (agentd
RFC 0020 §5):
...
`agentd://run|subagent/{handle}`) is the **agent's private concern, documented in the
...
**distillate-only invariant holds**: the agent streams *status*, never partial artifacts (agentd
...
- **The missing primitive is P-trace.** Whether the agent **ingests** an inbound `traceparent` *on
  ...
  self-MCP request and via `AGENTD_TRACEPARENT`, but not on the A2A surface the gateway uses
...
- **An HTTP server in the agent, durable history in the agent, or any auth in the agent.** agentd
...
- **agentd RFC 0005/0009 (the reference impl's contract spec)** — self-MCP surface + subagent model:
  `agentd://run|subagent/{handle}` (= a Task), the async-subagent machinery a `message/send` starts,
...
- **agentd RFC 0010 (the reference impl's contract spec)** — observability/health: W3C trace-context
  on by default, ingest on the self-MCP request + `AGENTD_TRACEPARENT` but **not** the A2A surface
  (the **P-trace** gap, §7).
