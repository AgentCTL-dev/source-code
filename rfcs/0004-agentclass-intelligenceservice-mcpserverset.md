...
spec:
  ...
  failover:
    ...
    breakerThreshold: 3                  # → AGENT_INTEL_BREAKER_THRESHOLD (agentd RFC 0018 §4.2) [branded; flagged]
    ...
    allDownBackoff: "1s..30s"            # → AGENT_INTEL_ALLDOWN_BACKOFF (agentd RFC 0018 §6) [branded; flagged]
...
the contract's `AGENT_INTELLIGENCE_TOKEN[_n]`/`_FILE` env (agentd RFC 0014 §6.4) —
...
- **Stable names for metric labels.** Each `endpoints[].name` is the **bounded,
  ...
  (agentd RFC 0018 §4.3/§11, contract ask P7) so the `agent_intel_endpoint_*`
...
- **agentd RFC 0016** — telemetry & lifecycle contract: the frozen `agent_intel_*`
