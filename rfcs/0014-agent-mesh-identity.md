...
> (agentd RFC 0014 §6 non-goals). This RFC adds **no** agent ask of its own; it
...
| `securitySchemes`/`security` | the gateway PEP (RFC 0013) | the auth the **gateway** enforces, not the agent (agent is auth-free, agentd RFC 0012 §3.8). |
