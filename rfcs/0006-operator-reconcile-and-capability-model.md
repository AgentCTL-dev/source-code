...
> `agentd://capabilities`), keys every rendering decision off the manifest's
...
3. **The capability model is two-path and the paths never cross.** **STATIC**
   ...
   time**. **LIVE** capability (the `agentd://capabilities` / `inventory` / `status`
...
`agentd.agentctl.io/managed`; this RFC uses the **proposed** `agents.x-k8s.io` group
...
reference-era `agentd.` spelling.) If the cache were
...
```
   ...
   (status time)       │  obtained by: node-agent reads agentd://capabilities /    │
...
`agentd://capabilities` (re-read on hot-reload/model-swap via the `updated`
notification), `agentd://inventory` (the live subagent tree), and `agentd://status`
...
- **Primary: a short-lived `CapabilityProbe` Job.** The operator renders a one-shot
  ...
  contract's static-capabilities invocation — reference impl: `agentd --capabilities`
...
tier is held back, not crash-looped. Capability *absence is never an error* (agentd
...
value); its *value* (`unix:/run/agentd/mgmt.sock` vs `vsock:5005`) is set by the
...
(agentctl RFC 0002 §6, RFC 0003 §3.2). The renderer carries the **`AGENTD_SHARD`
...
scale-down; that drain rides the pod's grace + the agent's SIGTERM handling (agentd
...
handleFinalizer(agent):
  ...
  2. wait for in-flight to drain (watch agentd://inventory active count / agentd_active_subagents),
...
AgentReconciler.reconcile(agent):                          [level-triggered; idempotent]
 ...
 10. snapshot := nodeAgent.observedSnapshot(agent)   // agentd://capabilities/inventory/status; tolerate nil
...
- **agentctl RFC 0003** — Agent & AgentFleet CRD schema + status contract: the `.spec`
  ...
  partition, the downward-API env set, the `AGENTD_SHARD` defect (P3), the KEDA
...
- **agentd RFC 0015** — management & control surface: the manifest schema and its
  static-vs-live emission (§5.2), `agentd://inventory`/`status` (§5.3/§5.4), the
...
- **agentd RFC 0019** — horizontal scaling: claim/shard ownership, the claim-release
  drain (§6) the pod-SIGTERM path runs, the `AGENTD_SHARD` defect (§4.2, contract ask
  P3).
...
A2A branch), **P3** (`--shard auto/N` — the `AGENTD_SHARD` defect, §6.3), **P5**
