...
> implementation** (`agentd`, whose contract is presently specified in agentd RFCs
...
- the **operator** reads `agentd://capabilities`/`inventory`/`status` to project
...
- the **telemetry** path scrapes `agentd://metrics` and tails `agentd://events`
  (agentctl RFC 0010).
...
- **`surface` selects the contract surface; the descriptor *shape* is shared, the
  ...
  - **metrics/events** speak **Prometheus 0.0.4 text over HTTP** (and an events
    ...
    **separate socket** advertised via `surfaces.metrics` (not `surfaces.management`),
    ...
    Furthermore, **whether `agentd://metrics` is reachable over the management
...
instruction — reference impl: `--serve-mcp unix:/run/agentd/mgmt.sock` (the unix
serve form originates in agentd RFC 0005 §3.6; agentd RFC 0015 §3.1 generalizes it
to the vsock form) — where `/run/agentd` is a per-pod directory on a hostPath
...
instruction itself is an agentd-branded contract surface to neutralize — open
question (a).)
...
is byte-for-byte unaware it is on Kata — it binds a vsock listener exactly as agentd
...
sharing an `emptyDir` volume: the agent serves `unix:/run/agentd/mgmt.sock` on the
...
spec:
  containers:
    - name: agent
      ...
      # management socket). Reference impl spelling: the AGENTD_SERVE_MCP env (or
      # the --serve-mcp CLI flag, agentd RFC 0015 §3.1). It is an agentd-BRANDED
      ...
      env:
        - name: AGENTD_SERVE_MCP
          value: "unix:/run/agentd/mgmt.sock"             # contract: surfaces.management = "unix:/run/agentd/mgmt.sock"
        - name: AGENTD_POD_UID
          valueFrom: { fieldRef: { fieldPath: metadata.uid } }   # agentd RFC 0014 §6.4
      volumeMounts:
        - name: agentctl-sockets
          mountPath: /run/agentd
          subPathExpr: $(AGENTD_POD_UID)        # → host: /run/agentctl/sockets/<pod-uid>/
  volumes:
    - name: agentctl-sockets
...
/run/agentctl/sockets/                  (0711, node-agent SA owns)
  ├── f3c1…/  ── mounted as ──▶  /run/agentd/        ◀── agent binds mgmt.sock here
...
- Each pod mounts **only its own** `<pod-uid>/` subdir via `subPathExpr`
  (`$(AGENTD_POD_UID)`); a tenant pod cannot traverse up out of its mount, so it
...
> The sidecar-emptydir tier (§4.3) uses the same `unix:/run/agentd/mgmt.sock`
...
> implementation should ship e.g. `agentd --check-health PATH` (reads its own
...
agent *where* to bind its management socket — reference impl: `AGENTD_SERVE_MCP` /
...
repo (agentd RFC 0014–0016) and are agentd-**branded**. Recommended: extract them
...
(h) **Is `agentd://metrics` reachable over the management socket? (P4 — a contract
...
`agentd://metrics`; agentd RFC 0019 assumes it). Resolution is the contract ask
