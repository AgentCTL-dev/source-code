...
> supersedes-in-part **0002** (transport), **0008** (retired), **0009/0010/0013/0015**
...
>
> **Update (2026-07-10) — the intelligence + tools gateways were removed.** The
> **ModelGateway** and **MCPGateway** that 0021 realized, and the **`MCPServerSet`** CRD, are
> gone: agents dial model providers and MCP servers **directly** (keyless AAuth or a mounted
> key), and budgets are harness-tracked. **RFCs 0012 (Intelligence plane) and 0019 (MCP
> registration) were removed outright** as wholly obsolete; **0004/0021/0022/0023/0024/0025**
> carry banners recording the pivot. On the intelligence/tools egress model, the
> **direct-dial** design is authoritative.

> 🚀 **Update (2026-08-30) — the v2 agent-cloud track ([RFC 0026](0026-v2-agent-cloud-pivot.md)+).**
> The reference agent was rewritten again (durable-workflow runtime, AGPL relicense +
> version reset to a new v1.x; execution modes, clustering, and the served-MCP surface
> are **gone**), and the mission widened to a full **agent cloud** (per-user supervisor
> agents, brokered identity with OBO token exchange, an mcpg-based governed capability
> plane, tenancy, registries, lifecycle, multi-cluster). The architectural umbrella is
> [`docs/v2/ARCHITECTURE.md`](../docs/v2/ARCHITECTURE.md) (+ ADRs in
> [`docs/v2/adr/`](../docs/v2/adr/), plan in [`docs/v2/PLAN.md`](../docs/v2/PLAN.md));
> **RFC 0026 carries the authoritative supersession map** over 0001–0025. Note the
> gateway distinction: the *credential-static* gateways removed in the 2026-07-10 pivot
> stay removed; v2's gateways are **identity-injecting data planes** (ADR-0004) — a
> different animal, justified by per-user OBO credentials that no mounted key can provide.
...
were written** (Proposed), but **0012** (the intelligence plane) and **0019** (MCP
server registration + the secret-free MCP broker) were subsequently **removed** as
wholly obsolete (see the update above): what remains is the **0001–0018 core track**
below (minus 0012), plus **0020** (instruction sourcing & live delivery) — which
extends RFC 0003's `instruction` with a live-instruction resolver (§ "The
instruction-sourcing track", below). The
...
scaling plane (0011), the intelligence plane (0012, **withdrawn** — agents dial
providers directly), the A2A gateway & task store
...
| [0004](0004-agentclass-intelligenceservice-mcpserverset.md) | AgentClass, IntelligenceService (ModelPool) & MCPServerSet | Partially superseded | The ops/dev decoupling CRDs an `Agent`/`AgentFleet` points at: `AgentClass` (substrate tier + tenancy posture + contract-version pin home), `IntelligenceService` (ordered model endpoints, zero-secret-in-pod via the egress proxy), `MCPServerSet` (reusable per-tool-glob-tagged tool bundles); deterministic deep-merge-maps / replace-lists / ADD-MCP-servers override semantics. *(Superseded parts: the `MCPServerSet` bundle and the intelligence egress proxy were removed in the direct-dial pivot — MCP servers are now inline on `Agent.spec.mcpServers` and providers are dialed directly; `AgentClass` and `ModelPool` stay.)* |
...
| [0024](0024-aauth-delegation-remote-resources.md) | AAuth delegation — direct-dial remote MCP & the intelligence posture | Proposed (identity/delegation track) | The rewriting-proxy theorem (RFC 9421 covers `@authority`/`@path` ⇒ AAuth-mode MCP is **direct-dial**, the facade stays for credential-bearing servers); `auth.mode: aauth` (with 0019's broker tiers **removed**, this is now the primary MCP-auth path); admission coupling that finally gives `capabilities.egress` enforcement semantics + tiered egress rendering (honest about vanilla-NetworkPolicy limits); the governance shift (per-agent authorization/attribution at the resource, nothing stored); the public-issuer requirement; the intelligence posture — the modelgateway was **removed**, so AAuth is now the direct-dial identity to the provider pool (inbound and outbound) and budgets are harness-tracked (RFC 0025). |
...
| [0026](0026-v2-agent-cloud-pivot.md) | **agentctl v2 — the agent-cloud pivot (umbrella)** | **Proposed (v2 track)** | Scope of the v2 re-founding, the ACC v2 contract re-baseline (config/workflow schemas + A2A profile + exit intents + store profile replace `surfaces{}` negotiation), and the authoritative supersession map over 0001–0025. |
| [0027](0027-supervisor-agents-and-control-mcp.md) | Supervisor agents & the control MCP | Proposed (v2 track) | One stock-agentd supervisor per user; instruction layering (system→org→user, user layer inert to directives); the `control.*` tool surface whose authority is OBO-derived from the acting human; cluster subagents as first-class child Agents. |
| [0028](0028-identity-service.md) | agentctl-identity: custody, exchange, agent identity | Proposed (v2 track) | The standalone broker: external-IdP federation, grant custody (envelope-encrypted), RFC 8693/ID-JAG on-behalf-of exchange, the AAuth agent-provider role (secret-free federated enrollment via projected SA tokens), per-(user,agent) A2A principal minting. Supersedes 0014; amends 0023–0025. |
| [0029](0029-a2a-gateway-v2-and-exposure.md) | A2A gateway v2 & exposure | Proposed (v2 track) | The unified A2A endpoint (`orgs/<org>/agents/<name>`), OIDC-in / mTLS+principal-bearer-out routing that makes addressed gates and per-user quotas work natively, SSE passthrough, and `hooks.<domain>` webhook exposure. Supersedes 0013. |
| [0030](0030-capability-egress-and-obo.md) | Capability egress: per-org mcpg with identity injection | Proposed (v2 track) | The per-organization mcpg gateway: AAuth-verified agent callers, `cred://` per-request OBO token injection via identity, registry-rendered catalogs/quotas/gates/audit; direct dial stays for service-credential targets and intelligence. |
| [0031](0031-foundational-mcp-services.md) | Foundational MCP services on mcpg | Proposed (v2 track) | `state.*` (agentd's checkpointer profile, seq-CAS on Postgres — `store.class: managed`), `artifacts.*`, `work.*` (the fleet fabric), `sandbox.*` (execution as a governed service), `hitl.*` + the gate bridge. |
| [0032](0032-registry-and-provisioning.md) | Registry & config projection | Proposed (v2 track) | Scoped catalogs (system→org→group→user; content shadows, security floors narrow-only) compiled into agentd's layered directory (`services.yaml` + `agentd.yml` + conventional folders); the admission ladder gains binary validation (`--validate-config`). Re-founds 0004's AgentClass. |
| [0033](0033-crd-api-v1alpha2-and-lifecycle.md) | CRD API v1alpha2 & the agent lifecycle | Proposed (v2 track) | The v1alpha2 kinds (Agent, AgentFleet, AgentTemplate, AgentClass, MCPService, Organization, Supervisor, Cluster) with conversion; reconcile on the rendered-directory model; the full verb set (pause/drain/backup/restore/migrate/upgrade/reset/delete) with durability invariants. Supersedes 0003. |
| [0034](0034-fleets-and-scaling-v2.md) | Fleets & scaling v2 | Proposed (v2 track) | Partitioning as a renderer concern (static overlays + ordinal-0 singletons / dispatcher / workqueue leases on `work.*`), guarded re-partition, the scaler on live signals (`agent_inbox_pending`, pressure, queue depth) with breaker damping and scale-from-zero, per-fleet budgets. Supersedes 0011; supersedes-in-part 0022. |
| [0035](0035-multi-cluster.md) | Multi-cluster: hub & spokes | **Draft (v2 track, deferred)** | Hub (apiserver/identity/gateway/registry) + autonomous spokes; one AAuth trust domain; gateway-federated routing; migration as backup→restore. Held at Draft until the single-cluster phases land. |

These seventeen are P0-disciplined and mutually consistent: they share the tier names
...
Tier A; 0011 reads the 0003 scaling fields and drives KEDA; 0012's egress proxy (**withdrawn** — direct-dial now) was
...
(the 0012 proxy, since **withdrawn**), resolves the deferred PKI/egress/secret seams, and is the home of
...
## The instruction-sourcing track (0020)

> **`0019` (MCP server registration + the secret-free MCP broker) was removed** — see the
> update banner at the top. Its `MCPServerSet` CRD and MCPGateway broker are gone: MCP
> servers are declared **inline** on `Agent.spec.mcpServers` and dialed **directly** (keyless
> AAuth per [RFC 0024](0024-aauth-delegation-remote-resources.md), or a token mounted on the
> pod). Only **0020** remains from this track.

**0020** generalizes RFC 0003's inline `instruction` into an
...
provenance-recorded. 0020's `mcpResource` instruction is now
authenticated by the agent's **own direct MCP dial** (keyless AAuth per RFC 0024, or a
mounted token), not an off-pod broker. It preserves
every core-track invariant — **P0**, secret-free pod, out-of-node-agent egress,
attested identity, hostile-multitenancy isolation — and does not gate an MVP
milestone (it is additive; inline/no-MCP agents run untouched). The design research
behind it is persisted, fully sourced, in
...
**drops the modelgateway** — agents dial the provider pool directly (keyless AAuth or a
mounted key), so the former in-path meter is gone and budgets are harness-tracked. **0025**
closes the loop that direct paths open: budget becomes a property of the *agent* — declared on the CR, enforced
...
The **0020 instruction-sourcing track** extends it beyond that original brainstorm
enumeration (prompted by the MCP-authentication + instruction-sourcing work — its
MCP-registration half, **0019**, was later removed; design
...
written (Proposed) — **0012 and 0019 have since been removed as wholly obsolete** — and **[RFC 0021](0021-contract-2.0-network-substrate-pivot.md)** then
...
/ lifecycle / conformance capstone (0015–0018), and the instruction-sourcing track
(0020: instruction sourcing & live delivery — its sibling **0019**, MCP registration/identity/auth, was **removed**). The forward
...
and conformance seams the earlier RFCs deferred; and 0020 realizes the `instructionSource`
(RFC 0003) seam the CRD track reserved (the `MCPServerSet` seam from RFC 0004 §5 was **dropped** — MCP servers are inline + direct-dial now). **The
