...
- **Governable** — tokens cost money. The contract emits trustworthy token counts
  (`agent_tokens_total{model,type}`, never an estimate, agentd RFC 0016 §4.3) and a
...
6. **Token metering is two-tier; `agent_tokens_total` is the billing authority.**
   The frozen, never-estimated `agent_tokens_total{model,type}` (agentd RFC 0016 §4.3)
...
1. **`fallbackDirect` (RFC 0004 §4.2)** appends a **non-proxy** endpoint to the
   agent's `AGENT_INTELLIGENCE` list, so a proxy outage fails *over* (agentd RFC 0018
...
|---|---|---|---|
| `agent_intel_endpoint_up` / `_active` / `_latency_ms` / `agent_intel_calls_total{result}` / `agent_intel_all_down` (frozen, agentd RFC 0016 §4.3) | the **agent** | a **pool** (proxy endpoint), labeled by stable name (P7) or list index | the **pod↔proxy** hop and across-pool failover state |
...
`agent://intelligence` (agentd RFC 0018 §4.4, the agent's pod↔proxy view) plus the
...
- For **any dialect the negotiated agent advertises it supports in-binary**, the
  **dialect stays the agent's**. `endpoints[].dialect` (RFC 0004 §4.1) tells the agent
  ...
  `anthropic` dialect **values** (agentd RFC 0006 §4, `AGENT_INTELLIGENCE_DIALECT`
...
- For the **long tail the agent does NOT advertise** (Gemini, Vertex-native,
  ...
  set — the providers agent deliberately keeps *out* of its binary, agentd RFC 0006
...
agent emits **tokens, never currency** (agentd RFC 0016 §4.3); cost = tokens × a
...
|---|---|---|---|
| `agent_tokens_total{model,type}` (frozen, agentd RFC 0016) | contract-frozen, **never estimated** (absence is `0`, not a guess), per-pod, already scraped (agentctl RFC 0010) | `type ∈ {in,out}` only — **no cache-tier breakdown** | **the billing authority** for fleet/tenant governance |
...
`agent_limit_exceeded_total{limit="tree_tokens"}`, agentd RFC 0016 §4.3) and on `once`
