...
> metrics, and the exit-code contract. **The reference implementation (agent) is
> the first agent to implement these**; its branded contract surfaces
> (`AGENT_INTELLIGENCE`, the `AGENT_INTELLIGENCE_TOKEN[_n]`/`_FILE` env family, the
> `AGENT_INTEL_*` failover/breaker env family, the `agent_intel_*` / `agent_tokens_total`
> metric series, `agent://intelligence`, `--model-swap`, `--intelligence-names`) are
...
> a repointed endpoint with fresh breaker state and zero restart (agent RFC 0018
...
> model replicas (the agent never load-balances — sticky-primary only, agent RFC
...
the data-plane side to a deliberately thin primitive (agent RFC 0006): the agentic
ReAct loop, running inside a subagent process, must reach **exactly one** LLM
endpoint named by `AGENT_INTELLIGENCE`, dialed fresh per call (`Connection: close`),
...
`vsock:` channel to a host service (agent RFC 0006 §2). The agent does not
...
- **Resilient** — a fleet of agent pods whose model is supplied by an in-cluster /
  ...
  circuit breaker, all-down back-pressure, and runtime hot-swap (agent RFC 0018), but
  it is **agentctl** that owns *which* endpoints exist, *where* they are, and *when*
  they move (agent RFC 0018 §1, §10 — "where is the model service is agentctl's").
...
- **Governable** — tokens cost money. The contract emits trustworthy token counts
  (`agent_tokens_total{model,type}`, never an estimate, agent RFC 0016 §4.3) and a
...
1. **Behind the one endpoint sits an egress proxy fronting the ModelPool.** The
   ...
   (b) the agent's `AGENT_INTELLIGENCE` list pointing at the **proxy** over the
   substrate, **keyless**. The agent's resilience machinery (agent RFC 0018) operates
...
3. **Two-level resilience, mapped onto the data/control split.** *Within* a pool
   ...
   *never* load-balances (agent RFC 0018 §10). *Across* pools the **agent** fails over
   (sticky-primary, per-endpoint breaker, all-down back-pressure, hot-swap — agent RFC
...
4. **Zero-secret-in-pod is the default and the only hostile-tenant-legal topology.**
   ...
   agent pod; the agent's endpoint carries no key (agent RFC 0006 §6, agent RFC 0012
...
5. **Endpoints are named, not indexed; dialects stay the agent's by default — and
   *which* dialects those are is read from the manifest, never assumed.** Each pool
   carries a **stable operator-assigned name** so the frozen `agent_intel_endpoint_*`
   ...
   `anthropic` dialect values, but that two-adapter inventory is an agent *design
   choice*, agent RFC 0006 §3, **not** a contract guarantee) and **translates only the
...
6. **Token metering is two-tier; `agent_tokens_total` is the billing authority.**
   The frozen, never-estimated `agent_tokens_total{model,type}` (agent RFC 0016 §4.3)
...
| Across-pool **failover / breaker / all-down / hot-swap** | **the contract** (agent RFC 0018) | agentctl supplies/moves the list; the agent runs the machine |
| Intelligence **wire / dialects / keyless dial / `usage`** | **the contract** (agent RFC 0006) | the proxy speaks the agent-facing half of this wire |
...
- **It is a per-node inference SPOF.** The agentic loop **blocks** on the model call
  (agent RFC 0006 §7, RFC 0007) — reasoning cannot advance until the model answers.
...
- **It collapses inter-pool failover into one shared-fate hop.** The contract's
  failover/breaker/all-down machinery (agent RFC 0018) operates *across* pool
  endpoints. Terminating multiple pool endpoints on one per-node process makes that
  machinery shared-fate and blinds the frozen `agent_intel_endpoint_*` metrics —
...
1. **`fallbackDirect` (RFC 0004 §4.2)** appends a **non-proxy** endpoint to the
   agent's `AGENT_INTELLIGENCE` list, so a proxy outage fails *over* (agent RFC 0018
...
3. **A total proxy outage degrades through the contract, not into a crash-loop.**
   ...
   all-endpoints-down semantics (agent RFC 0018 §6): a `once` Job exits
...
```
   ...
   ┌──────────────────────────────────┐             ┌──────────────────────────────────┐
   │ AGENT / CONTRACT (agent RFC 0018)│  ordered    │ endpoints[] (ORDER = failover)    │
...
|---|---|---|---|
| **Across pools** (primary→fallback, different model svc/region) | **the agent** | ordered list, sticky-primary, per-endpoint breaker, all-down backoff | agent RFC 0018 §3–§6 |
...
| **Executing** a swap (quiesce→switch→resume) | **the agent** | turn-boundary hot-swap, `--model-swap` policy | agent RFC 0018 §5 |
...
ordered `AGENT_INTELLIGENCE` list (one entry per pool, in failover order) and the
RFC 0018 failover/breaker env (`AGENT_INTEL_BREAKER_*`, `AGENT_INTEL_ALLDOWN_BACKOFF`
...
resilience code paths are inert (agent RFC 0018 §3.1) and the plane is exactly RFC
0006 behind a proxy.
...
- **Layer 1 — proxy-internal (within-pool), the agent sees nothing.** Repointing a
  ...
  **not** move, so `AGENT_INTELLIGENCE` is unchanged and there is nothing to reload
...
- **Layer 2 — across-pool (the agent's quiesce→switch→resume).** Adding, removing, or
  reordering a *pool*, or changing the **model**, changes `AGENT_INTELLIGENCE` /
  `model`. The operator re-renders the env (via a stable-named reloadable ConfigMap,
  agentctl RFC 0006) and signals a hot reload (agent RFC 0017); the agent executes
  the contract's **quiesce → switch → resume** at a turn boundary per `swapPolicy`
  (agent RFC 0018 §5): `finish-on-old` (default) lets the in-flight turn complete on
...
the agent only executes the primitive (agent RFC 0018 §5.3, "deciding to swap is
agentctl's").

### 4.4 Within-pool: what the proxy actually does

The proxy is the component agent RFC 0006 always assumed existed for everything its
two in-binary adapters do not cover ("fewer adapters, thinner binary, push provider
quirks to the gateway," agent RFC 0006 §2.4). It MUST:

- **Load-balance across `backends[]`** with weighted or least-latency selection
  (agentctl's choice; the agent contract forbids itself from doing this, agent RFC
...
- **Terminate provider TLS** and hold the credential (§5). The agent dials the proxy
  ...
  TLS — agent RFC 0012 §3.8); the proxy makes the real `https://` egress.
...
- **Meter `usage`** per backend for the within-pool cost view (§7.1), and **never log
  ...
  header (the same never-logged discipline as agent RFC 0006 §6).

The proxy is **non-streaming agent-facing** (agent RFC 0006 is `stream:false`): if a
...
keep-alive state to the agent** — the agent dials fresh per call (agent RFC 0006 §7).
...
`agent_intel_endpoint_*` metrics mean** — they now measure the **pod↔proxy hop**, not
...
|---|---|---|---|
| `agent_intel_endpoint_up` / `_active` / `_latency_ms` / `agent_intel_calls_total{result}` / `agent_intel_all_down` (frozen, agent RFC 0016 §4.3) | the **agent** | a **pool** (proxy endpoint), labeled by stable name (P7) or list index | the **pod↔proxy** hop and across-pool failover state |
...
Dashboards stack the two: `agent_intel_endpoint_active{endpoint="opus-primary"}` says
...
`agent://intelligence` (agent RFC 0018 §4.4, the agent's pod↔proxy view) plus the
...
The contract's optional `GET /v1/models` discovery (agent RFC 0018 §5.4) surfaces
...
an anthropic-dialect pool (agent RFC 0018 §5.4). So:
...
- secrets are env/file-only behind the `resolve()` front door and are **never read
  from the config file** (agent RFC 0006 §6);
- the credential carrier (`Token`/`Secret`) is **structurally unserializable** — it
  ...
  log line, or a trace (agent RFC 0006 §6 / RFC 0012 §3.7);
- on the keyless tier the agent dials a substrate-local socket with **no key in the
  URI and no token env** (agent RFC 0018 §3.2 — "the list never carries a secret";
  agentctl RFC 0002 §10).

### 5.2 Where the credential lives, by topology

| `proxy.topology` | Credential mounted into | Agent pod env | Agent's `AGENT_INTELLIGENCE` → | Hostile-tenant? |
|---|---|---|---|---|
| `sidecar` | the in-pod sidecar (mounts `credentialRef`) | **no** `AGENT_INTELLIGENCE_TOKEN*` | `unix:/run/intel.sock` (substrate-local, keyless) | yes (cred off the agent container) |
| `node-local` | the proxy Deployment | **no** `AGENT_INTELLIGENCE_TOKEN*` | the proxy endpoint over vsock/unix-hostPath, keyless | **yes** (the zero-secret path) |
| `none` (direct dial) | **the agent pod** (`AGENT_INTELLIGENCE_TOKEN[_n]`/`_FILE`) | the provider key | the provider endpoint directly | **no** — admission rejects |

In the default proxy-fronted topologies the operator renders `AGENT_INTELLIGENCE` to
...
injects the contract's `AGENT_INTELLIGENCE_TOKEN[_n]`/`_FILE` env into the pod — the
...
- **At rest:** a Kubernetes `Secret` (or an external-secrets-projected file). The
  ...
  (agent RFC 0006 §6: k8s Secret mounts / Vault Agent sidecars rotate by replacing
...
`AGENT_INTELLIGENCE` list; a backend/credential move is a **one-object edit** of the
`IntelligenceService` (RFC 0004 §4.5).

### 6.2 Stable per-endpoint names (P7) — why metric labels survive reorder

The contract today labels endpoints by **list index** (`"0"`, `"1"`, …, agent RFC
0018 §4.3). That index **shifts** when an operator inserts a new primary ahead of the
old one — a routine swap — so every `agent_intel_endpoint_*` series silently
...
`--intelligence-names` (agent RFC 0018 §11), so the label is `endpoint="opus-primary"`
...
**P-dialects**, the ask boxed below), honoring agent RFC 0006 §2.4:
...
- For **any dialect the negotiated agent advertises it supports in-binary**, the
  **dialect stays the agent's**. `endpoints[].dialect` (RFC 0004 §4.1) tells the agent
  ...
  `anthropic` dialect **values** (agent RFC 0006 §4, `AGENT_INTELLIGENCE_DIALECT`
...
- For the **long tail the agent does NOT advertise** (Gemini, Vertex-native,
  ...
  set — the providers agent deliberately keeps *out* of its binary, agent RFC 0006
  §2.4/§4), the proxy **is** the gateway agent RFC 0006 always assumed: it translates
...
> `intelligence_summary.toolmode` (agent RFC 0015 §5.2) but **no** dialect-capability
...
> second-vendor agent matches agent. **Contract ask: P-dialects.**
...
|---|---|---|---|
| `agent_tokens_total{model,type}` (frozen, agent RFC 0016) | contract-frozen, **never estimated** (absence is `0`, not a guess), per-pod, already scraped (agentctl RFC 0010) | `type ∈ {in,out}` only — **no cache-tier breakdown** | **the billing authority** for fleet/tenant governance |
...
The recommendation is binding for v1: **`agent_tokens_total` is the authority** for
...
and the contract has not added cache types to `agent_tokens_total`, revisit making
...
**contract limits box** (`maxTokens`, `treeTokenBudget`, agent RFC 0015 §5.2), set by
...
exhausted the agent refuses (`agent_refusals_total{reason="budget"}` /
`agent_limit_exceeded_total{limit="tree_tokens"}`, agent RFC 0016 §4.3) and on `once`
exits **`EXIT_BUDGET(7)`** (agent RFC 0016 §5, policy: default `Count`, operator may
...
- **Observe:** the per-tenant/per-fleet rollup `agentctl:tokens:rate5m = sum by
  (namespace, tenant, model, type) (rate(agent_tokens_total[5m]))` (agentctl RFC 0010
...
- **Throttle (control-loop, reactive):** as a soft budget is approached, agentctl
  ...
  **only when `lame-duck` is in the negotiated agent's `surfaces.operator_tools`** (the
  authoritative operator-tool list a build exposes, agent RFC 0015 §5.2; the lever is
...
2. **A clean budget-exhausted signal — contract ask P-cost.** When the shared
   ...
   **`once` → `EXIT_BUDGET(7)`** — and to stop the scheduler pointlessly retrying a
   ...
   is `Count` and IS retried** (it counts toward `backoffLimit`, agent RFC 0016 exit
   ...
   **`loop`/`reactive` → readiness back-pressure** (the pod flips not-ready
   and stops claiming work, exactly like all-down back-pressure, agent RFC 0018 §6 —
...
- The proxy (off-pod, its own netns) is the **only** component that touches the
  ...
  metadata-endpoint / allow-list controls (agent RFC 0012's SSRF posture, applied at
  the proxy because the pod can no longer reach anything to attack), per-tenant authz
  (§5.4), and metering (§7).
- The agent-side distillate firewall (the CaMeL-style reader/actor split, agent RFC
...
spec:
  ...
  failover: { breakerThreshold: 3, breakerCooldownSeconds: 5 }   # ACROSS-pool → agent RFC 0018 (branded env)
...
{
  "AGENT_INTELLIGENCE":       "vsock:<proxy-cid>:8081,vsock:<proxy-cid>:8082",  // pools in failover order
  "AGENT_INTELLIGENCE_NAMES": "opus-primary,opus-fallback",                     // P7 stable names → metric labels
  "AGENT_INTELLIGENCE_DIALECT": "anthropic",                                    // the agent's shipped adapter
  "AGENT_MODEL_SWAP":         "finish-on-old"
  // NOTE: NO AGENT_INTELLIGENCE_TOKEN* — the credential is NOT in this pod
}
...
Resulting metric two-tier view (§4.5): `agent_intel_endpoint_active{endpoint="opus-primary"}=1`
...
- **The across-pool failover/breaker/all-down/hot-swap algorithm.** Owned by the
  contract (agent RFC 0018). agentctl supplies and moves the list; the agent runs the
  machine. agentctl never re-implements it.
- **The intelligence wire, dialects, keyless dial, `usage` shape, secrets front
  door.** The contract (agent RFC 0006 / RFC 0012 §3.7). The proxy speaks the
...
- **Streaming intelligence end-to-end.** The contract is non-streaming
  (`stream:false`, agent RFC 0006); the proxy de-streams a streaming backend. A
  streaming agent-facing wire is a follow-up (agent RFC 0006 open item), not v1.
...
2. **Price-table freshness and ownership.** agentctl owns the price table (agent RFC
   ...
   cache `type`s to `agent_tokens_total`, does the proxy meter become the billing
...
3. **Streaming-to-buffered de-stream cap.** When the proxy de-streams a streaming
   ...
   MiB, agent RFC 0006 §3) and what happens on overflow — truncate, error, or fail
...
6. **Per-endpoint credential env keying on the direct-dial path.** The `topology: none`
   ...
   `AGENT_INTELLIGENCE_TOKEN_<n>`; the contract flags that **name-keyed** credentials
   may be preferable once stable endpoint names (P7) land (agent RFC 0018 §11,
...
- **agentctl RFC 0002** — substrate & transport abstraction: the keyless model-dial as
  ...
  plane embodies (§8), the per-tier reach the agent's `AGENT_INTELLIGENCE` dials over.
...
- **agent RFC 0006** — intelligence transport & wire: the one-endpoint dial, the three
...
- **agent RFC 0018** — intelligence transport resilience: the ordered `--intelligence`
  ...
  discovery (§4.6), the frozen `agent_intel_*` semantics + index-vs-name label open
  item (P7, §4.5/§6.2), `agent://intelligence`, and "agent never load-balances; where
  the model is, is agentctl's" (§4.1).
- **agent RFC 0016** — telemetry & lifecycle contract: the frozen
  `agent_tokens_total{model,type}` (the billing authority, §7.1), the never-estimate
  honesty, the refusal/limit counters, "tokens not currency," and the `EXIT_BUDGET(7)`
  / `EXIT_INTELLIGENCE(4)` exit codes (§7.2/§7.4).
- **agent RFC 0012** — security posture: the `Secret`-has-no-`Serialize` invariant the
...
- **agent RFC 0017** — declarative config & hot reload: the reload trigger the
...
- **agent RFC 0007** — agentic loop: the loop **blocks on the model call** — the basis
...
keying reconcile (index vs stable name) is the agent RFC 0018 §11 open item (agentctl
RFC 0004 OQ6, §Open 6). The agent-branded contract surfaces this plane resolves into
(`AGENT_INTELLIGENCE`, `AGENT_INTELLIGENCE_TOKEN[_n]`/`_FILE`, `AGENT_INTELLIGENCE_NAMES`,
the `AGENT_INTEL_*` failover/breaker family, `AGENT_MODEL_SWAP`/`--model-swap`,
`--intelligence-names`, `agent_intel_*`, `agent_tokens_total`, `agent://intelligence`)
