...
> **reference implementation's** contract spec (agent RFCs 0014–0020); those are
...
| `schedule` | `{cron, timezone?}` | cond. | agent RFC 0008 §3.1.4 (schedule mode; internal cron is UTC-only) | the CronJob cadence (§5.2). **Required iff `mode == schedule`** (CEL); forbidden otherwise. `cron` is a 5-field expression; `timezone` defaults to `UTC` (the contract's internal cron is UTC-only — a non-UTC `timezone` is honoured by the k8s CronJob clock, which is the one clock §5.2 keeps). Mirrors the `subscribe`↔`reactive` treatment. |
...
literal in the CR (agent RFC 0015 §10 open item). The manifest's
...
impl's A2A binding (agent RFC 0020) but **not listed** in the frozen manifest
schema (agent RFC 0015 §5.2). This is the contract ask **P2** (a new manifest
...
static shard (agent RFC 0019), and the replica count is driven by an autoscaling
...
- **`claim` (recommended default, the only elastic regime).** Every pod is shard
  `0/1`; cross-instance ownership is the work-claim lease alone (agent RFC 0019
...
RFC 0016) and its scaling RFC (agent RFC 0019 §5) — a contract FIX (**P10**); the
...
(agent RFC 0008 §2.2) — so the same `updated{uri}` is processed **twice** for the
overlap window (agent RFC 0019 §1). Rendering avoids this two ways:
...
2. **Fleet:** any `AgentFleet` (claim or shard) has cross-instance ownership by
   ...
   is resolved by `work.claim` granting to exactly one (agent RFC 0019 §3.2 — the
...
agent's *own* internal cron (agent RFC 0008 §2.7). Running both fires the work
...
cron** (agent RFC 0008 makes them mutually exclusive). The CronJob is rendered
...
health state (agent RFC 0010 §3.7: the `--health-file` read by an exec probe is
the *default* daemon health surface; the reference impl ships **no HTTP server** by
default, agent RFC 0015 §1). On any networkless substrate (the hardened/portable
...
health surface (the reference impl's `--health-http ADDR`, agent RFC 0010 §3.7
...
live capabilities manifest (agent RFC 0015 §5.2), `agent://inventory`/`status`
(agent RFC 0015 §5.3/§5.4), and workload status. It is **not** a raw manifest
...
manifest's `build_features`, which agent RFC 0015 §5.2 defines as "compiled-in
...
1. **Trifecta-tag union across objects.** Two individually-"safe" `MCPServerSet`s
   ...
   Rule-of-Two per-spawn over each child's narrowed grant (agent RFC 0012 §3.2),
...
3. **Config-schema validation.** The rendered config file must validate against
   the **contract's config JSON Schema** (agent RFC 0017 §4.2 `--config-schema`),
   ...
   target image** (`--validate-config`, agent RFC 0017 §4.1) whose failure
...
`contract_version` (agent RFC 0014 §6.3) is negotiated at runtime, lives in
...
The operator injects the contract's **downward-API env convention** (agent RFC
...
These are **descriptive, never load-bearing** (agent RFC 0015 §6): the agent uses
...
reference impl's spelling of the contract token (agent RFC 0014 §6.4), so injecting
...
The contract's env convention lists `AGENT_SHARD="K/N"` (agent RFC 0014 §6.4),
and the scaling RFC derives `K` from the StatefulSet ordinal and `N` from
`.spec.replicas` (agent RFC 0019 §4.2). **This is unimplementable from a single
...
- **Graceful degradation is built into the shape:** a `contract_version` major
  ...
  managed by liveness + exit codes + logs only (agent RFC 0014 §7); a surface the
...
- **agent RFC 0015** — management & control surface: the manifest schema (§5.2),
...
- **agent RFC 0017** — declarative config & hot reload: the config-file schema
  `spec.config` renders to, `--validate-config`/`--config-schema` (contract ask P6),
  the reloadable-vs-restart partition.
- **agent RFC 0018** — intelligence transport resilience: the multi-endpoint
  binding `intelligence`/`intelligenceRef` projects.
- **agent RFC 0019** — horizontal scaling: the claim/lease (§3), shard predicate
  (§4), autoscaling signals (§5), drain-release (§6) behind `AgentFleet`; the
  `AGENT_SHARD` defect (§4.2, contract ask P3).
- **agent RFC 0008** — execution modes & reactive routing: the `mode` vocabulary
  and the internal-cron-vs-CronJob mutual exclusion (§5.2).
- **agent RFC 0011** — cloud-native contract: the exit-code table, drain
  choreography, and `RUN_ID` the rendering and status lean on.
- **agent RFC 0012** — security posture: the trifecta tag vocabulary (§3.1) and
  the `allow-trifecta` / `enable-exec` gates `spec.security` projects.
- **agent RFC 0007** — agentic loop & terminal status: the terminal-status
  vocabulary `.status` and the once-mode `podFailurePolicy` consume.
- **agent RFC 0020** — A2A interop over vsock: the A2A binding and the
