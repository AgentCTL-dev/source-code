...
*and* `--shard K/N` requiring `N` to be fleet-consistent immutable config (agentd
...
set** itself is unreconciled between the reference impl's frozen metrics (agentd
...
`metrics_schema`'s actual (possibly `agentd_`-prefixed) metric name, so the CRD
...
live capabilities manifest (agentd RFC 0015 §5.2), `agentd://inventory`/`status`
...
// Agent.status — curated, structural-only
{
  ...
  "contract": {                             // negotiated from the manifest (RFC 0015 §5.2)
    ...
    "agentVersion": "2.2.0",               // implementation build id (agentd_version for the ref impl)
...
keys exclusively off `surfaces{}`** — the contract's single discovery point (agentd
...
operator to branch on agentd cargo strings; an operator that needs to know whether
...
```yaml
# rendered into every agent Pod (the contract's env convention; AGENTD_* is the
...
env:
  ...
  - { name: AGENTD_POD_UID,       valueFrom: { fieldRef: { fieldPath: metadata.uid } } }
  - { name: AGENTD_POD_NAMESPACE, valueFrom: { fieldRef: { fieldPath: metadata.namespace } } }
  - { name: AGENTD_NODE_NAME,     valueFrom: { fieldRef: { fieldPath: spec.nodeName } } }
  - { name: AGENTD_POD_GRACE_SECONDS, value: "30" }   # == terminationGracePeriodSeconds (drain<grace)
  # per-endpoint intelligence credentials ride AGENTD_INTELLIGENCE_TOKEN[_n] / _FILE (RFC 0014 §6.4)
```
...
The `AGENTD_*` prefix is the **deepest residual coupling in this RFC** — it is the
...
`AGENTD_*`. **Normative commitment:** before GA this MUST be resolved one of two
ways (Open Question 2) — either the contract **freezes `AGENTD_*` as the stable,
...
### 9.1 The `AGENTD_SHARD` defect (a contract ask, not an agentctl bug)

The contract's env convention lists `AGENTD_SHARD="K/N"` (agentd RFC 0014 §6.4),
...
ordinals** — every replica would receive the *same* `AGENTD_SHARD` string — and
...
`K` itself from the ordinal in `AGENTD_POD_NAME` (`<name>-<ordinal>`). The operator
...
`AGENTD_SHARD="K/N"` to a shared `emptyDir` env file the agent sources — which
...
2. **The downward-API env prefix under a neutral contract.** The contract token is
   currently spelled `AGENTD_*` (the reference impl). If the contract is extracted
   into a neutral "Agent Control Contract" spec (P0 open question, agentctl RFC
   0001), should the env family be frozen as `AGENTD_*` (a stable contract token,
...
- **agentctl RFC 0001** — stack & contract-as-schema foundation: the
  contract-not-agentd (P0) framing, the codegen/conformance anti-drift strategy
...
- **agentd RFC 0015** — management & control surface: the manifest schema (§5.2),
  `agentd://inventory`/`status` (§5.3/§5.4), the operator tools, env identity (§6) —
...
- **agentd RFC 0019** — horizontal scaling: the claim/lease (§3), shard predicate
  ...
  `AGENTD_SHARD` defect (§4.2, contract ask P3).
...
**P3** (`--shard auto/N` — the `AGENTD_SHARD` defect), **P5** (run-outcome capture
