...
| `logs -f` (live tail) | `agentd://events` (agentd RFC 0016) | streaming read |
| `tree -w` (live) | `agentd://inventory` (§5.3) | streaming read |
| `top -w` (live) | `agentd://metrics` / `agentd_*` (ask **P4** — to be defined in agentd RFC 0005/0015) | streaming read |
   ...
   ── reconcile / drain / snapshot ── mTLS ──────►┌──────────────────────────────────────────────┐
      (operator client cert)                      │      node-agent  (DaemonSet, Tier A)            │
                                       ...
                                       (agentd RFC 0015 §7)               ▼ PeerOrigin::Management
                                                  ┌──────────────────────────────────────────────┐
                                                  │  conformant agent pod (the reference impl: agentd)│
...
| `agents/log` | `get` | `agentd://events` (agentd RFC 0016) | streaming read |
| `agents/inventory` | `get` | `agentd://inventory` (§5.3) — backs `tree -w` | streaming read |
| `agents/metrics` | `get` | `agentd://metrics` (ask **P4** — to be defined in agentd RFC 0005/0015; referenced by RFC 0019) — backs live `top` | streaming read |
...
4. **Audit at the PEP too.** The node-agent emits a management-action audit record
   ...
   `mgmt.invoked`-class vocabulary (agentd ask **P-audit** / **P-meta**; agentctl
...
`drain`/`cancel`/observe** (agentd ask P-attach-gate; the home of the multi-tenant
...
- **agentd RFC 0016 (the reference impl's contract spec)** — telemetry & lifecycle
  contract: `agentd://events` (live `logs -f`) backing the `agents/log` streaming-read
  subresource (§5.1). `agentd://metrics` (live `top`) is **not yet a defined resource**
...
`pause`/`resume` tools, §1); **P4** (`agentd://metrics` for the live `top`
