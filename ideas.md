...
> The control plane for fleets of **agent** instances on Kubernetes: a CLI, a
> `kubectl agent[s]` plugin, a Kubernetes operator, and an on-node bridge that
> provisions, supplies intelligence, scales, observes, and manages agent —
...
## 1. Thesis: agentctl owns policy, agent owns primitives

`agent` (the sibling repo) is the **data plane** — a single static ~1.1 MB Rust
...
| **0016** frozen metrics schema + exit-code contract + `agent://events` | dashboards, alerts, `podFailurePolicy`, `kubectl agents top/results` |
...
```
        ...
        │  • management bridge: kube-apiserver/CLI ⇄ agent self-MCP over vsock│
        │  • A2A gateway:  HTTP/SSE/auth/webhooks ⇄ agent A2A over vsock      │
        │  • telemetry:    scrape /metrics + agent://events over vsock        │
        └───────────────────────────────┬────────────────────────────────────┘
                                vsock (point-to-point guest↔host)
                                         ▼
        ┌──────────────────────  agent pods (data plane)  ──────────────────┐
        │   NO cluster network — vsock-out for intelligence, vsock-in for     │
        │   management + A2A. Static binary on scratch. Nothing to attack.    │
        └─────────────────────────────────────────────────────────────────────┘
```

The load-bearing idea is **vsock-everything**: an agent pod can run with *no
...
- **`Agent`** — one logical agent: its instruction/config, intelligence binding,
  ...
  the config (ConfigMap), injects the **downward-API env** agent expects
  (`AGENT_POD_NAME/UID/NAMESPACE`, `AGENT_NODE_NAME`, `AGENT_POD_GRACE_SECONDS`,
  `AGENT_SHARD` — RFC 0014 §6.4), and wires vsock for intelligence + serving.
...
capabilities — it reads `agent --capabilities` (RFC 0015) at admission and only
...
**Admission**: a validating webhook runs `agent --validate-config` (RFC 0017)
...
Verbs map to agent's operator surface (RFC 0015), proxied through the node-agent:
...
- `kubectl agent <x> tree` — render `agent://inventory` (the live subagent tree).
- `kubectl agent <x> logs -f` — stream `agent://events` (RFC 0016).
...
The keystone. One per node, host-side, vsock-adjacent to every local agent pod.
...
3. **Telemetry collector** — scrapes each pod's metrics + tails `agent://events`
...
Because agent serves **real A2A over vsock** (RFC 0020), the gateway is a **dumb
transport bridge**, not a protocol translator:

- Serves the public A2A surface on the network (HTTP/JSON-RPC + SSE + webhooks +
  OAuth/OIDC/mTLS) — all the heavy A2A machinery agent deliberately lacks.
- **Projects the Agent Card** from agent's capabilities manifest (`surfaces.a2a`,
...
- Bridges A2A JSON-RPC frames HTTP/SSE ⇄ **vsock** to the pod, emitting the exact
  ...
  names agent implements. Streaming: agent writes a stream of same-`id`
...
- Is the **policy-enforcement point**: it authenticates the client, enforces
  ...
  history** (`ListTasks`) — agent serves only *live* tasks. A trusted forwarded
  request crosses vsock with no in-band auth (the transport is the boundary).

Symmetry worth stating: agent dials A2A *out* (delegation, RFC 0020 §3) over
...
- **Scale (reactive fleet).** `AgentFleet` → StatefulSet (shards) + KEDA
  `ScaledObject` on `agent_pending_events` (RFC 0016/0019). Replicas claim work
...
- **Rolling update / node drain.** `kubectl agent <x> lame-duck` → `/readyz` 503,
  in-flight bleeds off (watch `agent://inventory`) → `drain` → clean exit 0 (not
...
- **Observe.** `kubectl agent <x> tree`/`logs`/`top` read inventory/events/metrics
  over vsock; the operator aggregates fleet-wide views (agent never does).
- **Mesh.** An external A2A client (any vendor) discovers a fleet's Agent Card,
  `SendMessage`s a Task → gateway → vsock → agent run → distillate streamed back.
  An agent agent delegates *out* to a peer via `--a2a-peer` → gateway → mesh.
...
- **Language: Go** for the operator, CLI/kubectl-plugin, node-agent, and A2A
  ...
  the kubectl-plugin and KEDA-scaler SDKs, mature vsock + HTTP/SSE libs). agent
...
- **The contract is generated, not hand-copied.** `pkg/agent/` is the single typed
  ...
  set, the metrics schema (RFC 0016), the config schema (`agent --config-schema`,
...
1. **MVP — manage what exists.** `pkg/agent` vsock client for the **management**
...
5. **A2A gateway.** The HTTP↔vsock A2A bridge + Agent Card + auth/webhooks (RFC
   0020) — agent's A2A surface (server + streaming + client) already shipped, so
...
- **vsock CID allocation** per pod on a node (the node-agent's scheme; agent just
