...
//! mTLS client for the gateway → agent hop.
//!
//! The agent's control surface — `POST /mcp` on port 8443 — requires a CA-signed
//! **client** certificate. Presenting it authenticates the caller as the
//! `Management` origin, the only origin allowed to drive the agent's
//! capabilities/A2A/stream verbs, so only the control plane (the apiserver, this
//! gateway) can reach them. We present our client identity from
//! `/etc/agentctl-client/tls/{tls.crt,tls.key}` and verify the agent's server
//! cert against `/etc/agentctl-client/tls/ca.crt`.
//!
//! We deliberately DO NOT check the server hostname: the agent is addressed
...
/// A `ServerCertVerifier` that verifies the agent's cert chains to our CA
/// but does NOT check the server name (the agent is reached by dynamic pod
...
impl ServerCertVerifier for CaServerVerifier {
    fn verify_server_cert(
        ...
        // agent has no stable DNS name, only a churning pod IP. mTLS + this
...
/// Build the mTLS reqwest client for the agent hop. Built once and shared
