...
- **A2A mesh (rest of 0013/0014)**: fleet-level *single* Agent Card (one card per `AgentFleet`) + card **signing** (JWS/JWKS) + federation; **durable task store** (persist tasks/history/`tasks/list`) + **webhooks** (push-notification config); resumable SSE. (Done: the gateway, per-agent card, `message/send`, **`message/stream` live SSE**, `tasks/get`/`tasks/cancel`, and the **`GET /agents` discovery registry**.)
