...
- **Hardening** (rest of 0015): pod→socket attestation (`SO_PEERCRED`, 0002 §7) → turns the intelligence plane's header-asserted identity into *attested* identity; the Kata-hybrid vsock substrate tier; NetworkPolicy *enforcement* (manifests shipped; needs Calico/Cilium — kindnet ignores them); cert rotation (cert-manager/SPIFFE). (Done: node-agent control-API **mTLS** + egress/tenant NetworkPolicy manifests.)
