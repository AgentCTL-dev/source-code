...
> ask (§14).
>
> **Inputs synthesized here:** the agentd control-plane contract track (RFCs
...
- **§2–§12** walk every element of agentctl. Each section has: the recommended
  ...
  agentd contract touchpoints, the NEW agentd primitives the design needs, and
...
RFC is written against "a conformant agent" / "the contract," naming agentd only
as the reference implementation in examples. **Open question P0 raises:** the
contract is presently authored inside the agentd repo (agentd RFCs 0014–0020). It
...
| **Stack (D2)** | **Rust for all five components (kube-rs), one control-plane ecosystem.** | **Overrides** (analysis said Go) | The **"shared wire crate with agentd" rationale is void by P0** — importing a data-plane crate is the forbidden dependency. Rust now stands on **team / single-control-plane-ecosystem fit**, not coupling to agentd. **Two hard preconditions:** (1) **CC — the contract is published as language-neutral, machine-readable schemas** (manifest + config JSON Schema, the frozen metrics schema, the management MCP profile, the `a2a.*` method set); agentctl **codegens its own typed client** from these and never links a data-plane crate. Today the reference impl builds the manifest `json!`→`Value` (deliberately, for secret-safety) and `--config-schema` is unbuilt — so CC is real work, but it is **contract work, not agentd-coupling**, and it is what makes the contract portable to other agents. (2) The team **owns the kube-rs gaps**: webhook **cert rotation**, thinner **conversion-webhook** tooling, the **KEDA external scaler in tonic/gRPC**, a **Krew/kubectl plugin in Rust**. None blockers; all real work controller-runtime would give free. |
| **Tenancy** | **Hostile multi-tenancy in v1.** Untrusted tenants share the cluster from day one. | **Overrides** (analysis said trusted-tenant for MVP) | **Elevates the whole security plane (§10) into v1** and forces: (a) **Kata hybrid-vsock is mandatory** for any tenant workload (the microVM kernel boundary is the only real isolation — confirmed in D1); the stock-unix tier is then dev/single-tenant only. (b) **Attested pod-UID→socket mapping** must be solved *first* — on hostPath, the node-agent must prove which pod owns `/run/agentd/<pod>.sock` and prevent socket-squatting (`SO_PEERCRED`/UID-mapping, or rely on the per-VM uds that Kata isolates naturally). (c) **Per-tool attach/inject gating** (agentd ask **P-attach-gate**) so a tenant can `drain`/`cancel`/observe but not *puppet* a neighbour. (d) **Per-tenant A2A gateways + secret isolation.** |
| **v1 scope** | **All planes in v1:** Observe + Scale + A2A gateway/mesh + Intelligence ops/cost. | **Overrides** (analysis proposed a thin "manage what exists" MVP) | v1 is **the full platform, not an MVP**. The cross-repo critical path (§14) becomes the gating risk: v1 now depends on **~12 agentd primitives/fixes** (P2, P3, P4, P5, P7, P9, P10, P12, P-meta, P-seq, P-trace, P-cost, P-attach-gate). **agentd repo work must lead agentctl work** for each plane. Several asks are *defect fixes* (P3, P4, P10, P12), not features. |
...
- **CC (Contract-as-Schema)** — the contract published as language-neutral,
  ...
  conformant agent codegen/validate against. Replaces the earlier "agentd-wire
  crate" framing, which would have coupled agentctl to agentd (forbidden by P0).
...
**Verified against agentd source (2026-06-27) — refines the AW ask:**

- agentd ships **only a TypeScript SDK** (`sdk/typescript/`); crates are just
  `agentd` + `agentd-conformance`. There is **no Rust wire crate** to import.
- The manifest builder (`crates/agentd/src/capabilities.rs::manifest`) returns
  ...
  preserving agentd's invariant that secrets are structurally unserializable.
...
- `OPERATOR_TOOLS = ["drain", "lame-duck", "cancel"]` — **`pause`/`resume` are
  ...
  them ⇒ add **P-pause** to §14 (new agentd ask) or drop pause/resume from v1.
- `CONTRACT_VERSION = "1.0"`; agentd refuses an instance whose *major* it does
...
`agentd` is the **data plane**: a single static ~1.1 MB Rust binary that runs the
...
**agentd exposes primitives; agentctl owns policy** — is the entire architecture
...
> on capabilities agentd does not yet expose.** The honest design is a tiered
> substrate with a portable default, a node-agent split into a bounce-safe
> control tier and an HA data-path tier, and a small, prioritized set of agentd
...
| **D2** | Go vs Rust | **DECISION (2026-06-27): Rust, all five, one ecosystem with agentd** — overriding the analysis below, which recommended Go. The override is sound *only with* the two preconditions in §0.6 (agentd ships `agentd-wire` + `--config-schema`; team owns the kube-rs gaps). The Go analysis is retained below as the record of what Rust must compensate for. | (overridden) |
...
1. **PRIMARY / MVP — unix-socket on a hostPath, bridged to the host DaemonSet.**
   Runs on any stock runc/containerd cluster permitting hostPath. agentd serves
   `--serve-mcp unix:/run/agentd/<pod>.sock` on a shared volume; the host
   DaemonSet reaches it. Bridge is off-pod (agentd holds no secrets, the
...
3. **MOST-PORTABLE — per-pod sidecar over an emptyDir unix socket** where
   ...
   **note honestly that pod containers share the netns, so a networked sidecar
   means agentd is NOT network-isolated** — only off-pod bridge variants remove
   reachability.
...
- **"No network = nothing to exfiltrate" is false.** The intelligence/model
  ...
  the agentd-side reader/actor distillate firewall (RFC 0012 §3.3). Market that,
...
- **Probes break on any networkless pod.** kubelet httpGet/tcpSocket/grpc probes
  ...
  probes traverse CRI. agentd ships `--health-file` (RFC 0010 §3.7) as the
  default exec surface, but a scratch image has no shell/`cat` to read it. This
  needs an agentd primitive (§14, P1) and is a blocking dependency for the
...
agentctl and agentd unable to drift." The Go-vs-Rust analysis **verified this is
illusory against agentd's actual source**:

- `crates/agentd/src/capabilities.rs` builds the manifest with the `serde_json::json!`
  ...
  contract even promises (`agentd --config-schema`, RFC 0017) is **not
  implemented yet**.
...
- agentd's own blessed anti-drift pattern is the **black-box conformance suite**
  (`crates/agentd-conformance`, "never linking the agentd library"), and the only
...
mature, used by Firecracker tooling; goroutine-per-conn maps onto agentd's own
...
agentd commits to and ships a published `agentd-wire` crate
...
one-process — agentd's listener is thread-per-connection, so independent host
...
- **Tier A — control + telemetry DaemonSet.** Management bridge + telemetry
  ...
  (agentd survives, RFC 0015 §8 guarantees clean re-read on reconnect); a crash
...
- **Intelligence proxy: keep it OUT of both tiers** (see D-adjacent, §6). Prefer
  ...
  separate node-local Deployment; always keep a fallback endpoint in agentd's
...
nodes; (b) durability must survive node/pod loss (agentd is stateless,
...
physically pinned to the node where the agentd pod runs (vsock is point-to-point),
...
- **The relay must own the vsock task stream for the full lifecycle independent
  of any HTTP client.** agentd delivers the distillate **exactly once** as a
  ...
  consumer. We need an agentd primitive to **re-read a terminal distillate by run
...
status:
  ...
  phase: Running
  contract: { version: "1.0", agentdVersion: "2.2.0", compatible: true }
  buildFeatures: [serve-mcp, vsock, metrics, events, hot-reload]
  manifest:                           # CURATED projection (not the churny full doc)
    mode: reactive
    surfacesAdvertised: { management: "unix:/run/agentd/mgmt.sock", metrics: ":9090", events: true }
...
1. **CRD versioning: do NOT promise a `v1alpha1 → v1beta1 → v1` additive ladder
   ...
   keep the **agentd `contract_version` negotiation in `AgentClass`/`status` —
...
2. **"CEL, no webhook" is false advertising — a validating webhook (or operator
   ...
   `agentd --validate-config`. Be honest that the webhook is a first-class
...
3. **Trifecta is an Agent-level decision, not per-MCPServerSet.** Two
   ...
   `allowTrifecta`) in the webhook/pre-render. **More importantly:** agentd
...
| `schedule` | **CronJob of `mode=once` Jobs** | Pick ONE clock: k8s CronJob → agentd `once` per fire. Do NOT also run agentd's internal cron (RFC 0008 makes them mutually exclusive). |
...
### 2.4 agentd contract touchpoints
...
### 2.5 New agentd primitives needed
`surfaces.a2a` manifest key (P2); per-endpoint model arrays in the manifest (P7);
ratification of the downward-API env family incl. the `AGENTD_SHARD` defect (P3);
...
cluster + the agentd manifest, never from the triggering event; write `.status`
...
1. **STATIC, pre-pod (rendering + admission):** resolve `agentd --capabilities`
   and `agentd --config-schema` from the **image digest** via a `CapabilityProbe`
...
2. **LIVE, runtime (status reflection):** the node-agent reads
   `agentd://capabilities`/`status`/`inventory` and publishes an observed
...
binary exec on tenant images)** → an `agentd --validate-config` **init-container
running the EXACT target image** as the authoritative ground truth (exit 2 →
CrashLoop fast-fail).

**Red-team corrections:** do NOT shell tenant-specified `agentd` images
...
`AGENTD_SHARD` from the ordinal), so it validates the *file+flag* layers only and
...
unbuilt agentd primitives today (§14, P6).
...
the capability cache, the probe Jobs) carries the `agentd.agentctl.io/managed`
...
### 3.5 agentd touchpoints / primitives / open questions
...
channel), holds one long-lived MCP management connection per local agentd pod
...
Tier A node-agent Pod (DaemonSet, control-tier, privileged for discovery)
  ...
  ├── conn manager  : 1 long-lived MCP conn per local agentd pod (unix-hostPath | vsock)
  ...
  └── telemetry     : /metrics re-export + agentd://events tail → stdout/Loki
```
...
observability+control gap on ONE node and **zero data-plane impact** — agentd
...
- **Run-report collection is a real responsibility, not an afterthought.** For
  `once`/Job pods the report (`--report-file` / `agentd://run/{run_id}`) vanishes
...
### 4.4 agentd touchpoints / primitives / open questions
...
- **`mode: claim` (RECOMMENDED DEFAULT, the only elastic regime).** Every pod is
  ...
  `agentd_active_subagents`) gives graceful victim selection that a StatefulSet
...
- **Scale-from-zero cannot read agentd's per-pod metric.** `agentd_pending_events`
...
- **Metric-name drift must be reconciled.** RFC 0016 freezes
  `agentd_pending_events`/`agentd_reaction_lag_ms`; RFC 0019 §5 uses
  `agentd_reactive_backlog`/`agentd_saturation`/`agentd_tokens_per_sec`/`agentd_claims_lost_total`
...
- **`AGENTD_SHARD=K/N` is unimplementable from one StatefulSet pod template.**
  ...
  RFC 0019 §4.2 convention.** Require an agentd primitive: `--shard auto/N`
  deriving K from `AGENTD_POD_NAME`'s ordinal (P3). Interim: an initContainer
  shim (needs non-scratch).
- **`agentd://capacity` and `agentd://metrics` are referenced by RFC 0019 but
...
the probe invokes the agentd health verb (P1), not a fabricated `--health-probe`.
...
with a strict two-level division mapping onto the agentd/agentctl split:
...
- **Inside a pool = the proxy's job.** A pool is a set of interchangeable
  ...
  circuit breaker. This is deliberate: RFC 0018 §10 says agentd never
  load-balances (sticky-primary only), so *something* must.
- **Across pools = agentd's job (RFC 0018).** The operator hands each Agent an
  ordered list of pool endpoints → `AGENTD_INTELLIGENCE=...,...`; agentd's RFC 0018
  failover/breaker/all-down machinery operates across pools.

`ModelPool` keeps provider secrets off the agentd pod (keyless dial in the vsock
tier; the proxy injects the upstream key), presents a uniform openai-compatible
face (agentd runs `DIALECT=openai` for all pools; the proxy translates), and is
...
- **"Hard budget enforcement host-side" is false for multi-pod Agents/Fleets** (a
  ...
  an unbuilt `EXIT_BUDGET`/back-pressure agentd signal (P-cost). **Demote budget
...
- **Collapsing each pool to one stable vsock port blinds the frozen RFC 0016
  intel-health metrics** (`agentd_intel_endpoint_*` now measure the hop to the
  proxy, not the model). Require the proxy to **re-export per-backend health on
  its own series**, and document that agentd's intel metrics in this tier mean
  "pod↔proxy hop."
- **Cost model must be keyed by `{model,type}`** (input vs output pricing differ)
  ...
  source (proxy metering vs `agentd_tokens_total`).
- **Don't force `DIALECT=openai` for all pools blindly** — it orphans agentd's
  shipped anthropic adapter and re-implements tool-calling translation in a
  second codebase. Either keep per-pool dialect on agentd, or own a versioned
...
### 6.3 agentd touchpoints / primitives / open questions
Touchpoints: RFC 0006 (transport/wire, keyless dial, traceparent); RFC 0018
(failover/breaker/all-down/hot-swap, `agentd://intelligence`, intel metrics);
...
a protocol translator: agentd serves the live core methods over vsock; the
...
`tasks/cancel`, `tasks/resubscribe` → relay→agentd over vsock (terminal
...
- **Resolve the A2A version FIRST, do not present "dumb pass-through" and "pin
  ...
  several. If agentd serves the older names and the gateway serves v1.0, the
  gateway IS a version-translation layer — budget it as such (schema mapping +
  conformance against both versions) or pin agentd's served version and refuse
  others. Confirm the exact wire strings agentd's `a2a` feature registers (§14, P2).
- **Durability depends on the relay never missing the live distillate** (agentd
...
- **Honesty in the Agent Card:** advertise `capabilities.streaming` as
  ...
  `input-required`/`auth-required` interaction support truthfully (agentd is
...
- **Resumable SSE (`Last-Event-ID`) needs a monotonic per-frame seq agentd does
...
### 7.3 agentd touchpoints / primitives / open questions
...
- **`attach` is NOT contract-complete today.** It is `subagent.send` (RFC 0015
  ...
  session-target enumeration all need NEW agentd primitives (§14, P-inject,
...
- **`attach` target resolution is unsound as drawn:** `subagent.send` injects
  ...
  `agentd://session/{session_id}`, not handle `"0"`/`agentd://subagent/{handle}`.
...
agentctl (agentd has no session/auth model). The viewer "sees every steer"
...
- **Metrics:** node-agent as a per-pod relabeling scrape PROXY
  (`GET /proxy/<uid>/metrics` → `resources/read("agentd://metrics")` →
  byte-identical Prometheus text). Discovery via an operator-central Prometheus
  `http_sd` endpoint (one target per agentd pod, `__address__` = node-local
...
- **Events → logs:** **use the normal container-stderr → node log agent → Loki
  ...
  of pod networking). Reserve `agentd://events` (lossy ring) for live tail only.
- **Run outcomes:** Tier A subscribes `agentd://run/{run_id}` at pod-up, reads on
...
- **Traces:** the A2A gateway is the trace root (sets `_meta.traceparent` on the
  vsock frame); agentd carries `trace_id` through logs/events/report/MCP calls.
...
- **`agentd://metrics` is not in the authoritative resource lists** (RFC 0005/0015
...
- **Do not stamp `model` (a per-series label agentd already emits) or `metrics_schema`
...
Primitives: define `agentd://metrics` (byte-identical Prom text) + `agentd://capacity`
...
**two PEPs**: the node-agent management bridge and the A2A gateway. agentd
...
- **Keep every secret out of the agentd pod** (keyless intel dial in the vsock
...
- **Rule-of-Two:** surface operator-declared MCP tags in the CRD; gate the
  ...
  Agent-level union that refuses the safe reader/actor split — §2.2). agentd
...
- **`attach`/`inject` is not independently RBAC-enforceable** — `subagent.send`
  ...
  tenants, it needs an agentd per-tool gate within the Management profile
...
- **Egress is not closed by "no network":** the model channel is an irreducible
  ...
  the agentd reader/actor + distillate firewall.
- **The node-agent is a single high-value trust principal** (CRI socket =
  ...
  unprivileged.
- **`agentd --validate-config` in the webhook is version-skewed** — run it via
...
- **The MCP servers (not separate Pods) are the real ASI01 surface** — Kyverno
  `verifyImages` cannot verify them; bake them into the verified agentd image and
...
`pkg/agentd` (public, `go get`-able client mirroring agentd's TS-SDK precedent),
...
agentd binary over the unix-socket transport (the contract-clean dev fallback):

- Tier 1: `agentd --config-schema` → vendor pinned → generate config structs.
- Tier 2: manifest/inventory/status/capacity/operator-tool I/O — hand-author JSON
  Schemas until agentd ships schema emitters; conformance is the enforcement.
- Tier 3: metrics + exit codes — checked-in registry → generated constants,
  asserted present by scraping a live `/metrics`.

Pin codegen by `(agentd SHA + cargo feature-set + contract major.minor)` — NOT
SHA alone (manifest/surfaces/metrics are `cfg!`-conditional).

### 11.2 Red-team corrections

- **`agentd_reactive_backlog` does not exist in the frozen schema** — derive the
  Tier-3 registry by scraping a real agentd, not by hand-transcribing names
...
RFC 0016 (metrics/exit codes); RFC 0020 (A2A types + projection); `crates/agentd-conformance`.
...
`agentd-wire` crate. Open: canonical pinning unit; build-vs-download agentd in CI;
...
- **agentd build/version upgrade choreography** (canary, claim-safe rolling
...
- **The thesis** (agentd owns primitives, agentctl owns policy; no k8s in agentd;
...
- **A2A served by agentd over vsock; the gateway is a transport bridge + PEP**,
...
- **Admission: "validating webhook runs `agentd --validate-config`" → ladder**
...
- **Trifecta admission: blocking Agent-level union → advisory; gate the override**
  (agentd enforces per-spawn) (§2.2/§10).
...
These are primitives agentctl needs that agentd does not yet expose. Each notes
which milestone it gates and whether it is a NEW ask or a CONTRACT DEFECT/FIX.
Sequence agentctl work behind these.

| ID | Ask | Type | Gates |
|---|---|---|---|
| **P1** | Exec-probe health verb (e.g. `agentd --check-health PATH` that reads its own `--health-file` and exits 0/non-zero) — kubelet HTTP probes can't reach networkless pods and scratch has no shell to read the file | NEW | the entire isolation tier (every milestone on a networkless substrate) |
| **P3** | `--shard auto/N` deriving K from `AGENTD_POD_NAME` ordinal — `AGENTD_SHARD="K/N"` is unimplementable from one StatefulSet template; this is a defect in RFC 0014 §6.4 / RFC 0019 §4.2 | FIX | scale (shard mode) |
| **P4** | Define `agentd://metrics` (byte-identical Prom 0.0.4 text, pinned mimeType) and `agentd://capacity` (frozen schema) in RFC 0005/0015 — referenced by RFC 0019 but undefined | FIX | observe, scale (networkless metrics + victim selection) |
| **P6** | Implement `agentd --config-schema` (RFC 0017, not yet built) and a `--validate-config --config-only` mode tolerating absent env | FIX | config milestone + admission + Tier-1 codegen |
| **P10** | Reconcile the autoscaling metric names: RFC 0016 frozen (`agentd_pending_events`/`agentd_reaction_lag_ms`) vs RFC 0019 (`agentd_reactive_backlog`/`agentd_saturation`/...) into ONE set; add `agentd_saturation` to the frozen schema | FIX | scale (KEDA triggers) |
...
| **P12** | Umbrella decision: single canonical owner + frozen schema for `work.*` + `assign` + `agentd/claim_key` `_meta` (RFC 0015 vs 0019) | FIX | scale (coordination) |
| **P2** | `surfaces.a2a` manifest key (served A2A version + address, or false) — RFC 0020 references it but RFC 0015 §5.2 doesn't list it; commit agentd's `a2a` feature to specific wire method strings | NEW | A2A gateway |
| **P5** | Read-before-exit guarantee (or short post-terminal linger / read-ack) for `agentd://run/{run_id}` AND a way to re-read a terminal distillate by run handle — so a networkless once-mode result is not lost if the relay/collector blinks | NEW | A2A durability + run-report capture |
| **P-meta** | Descriptive caller/tenant `_meta` convention agentd echoes into its audit/event log (never re-verified, like downward-API identity) | NEW | A2A identity + management audit |
...
1. **agentctl-0001 — Stack & repo decision record.** Go for all five
   components; monorepo layout; Rust/hybrid revisit triggers; the `pkg/agentd`
...
5. **agentctl-0005 — CRD versioning & conversion policy.** Single-served-version
   + conversion webhook + SVM; alpha→beta→GA graduation; decoupled from agentd
...
16. **agentctl-0016 — CLI & kubectl-plugin grammar.** Three-faces packaging; cold/
    ...
    (scoped). (§8)
17. **agentctl-0017 — Release & lifecycle engineering.** agentd build rolling
...
18. **agentctl-0018 — Codegen & contract conformance.** Three-tier source-of-truth;
    `pkg/agentd` generation; SHA+feature pinning; conformance suite; E2E/chaos test
    strategy. (§11)
...
against shipped agentd primitives on a stock cluster** — i.e. it does NOT require
...
`pkg/agentd` client + conformance against a real agentd over **unix socket**;
...
runc/containerd against shipped agentd. **Blocking agentd asks: none for unix
...
**Blocking: P4** (define `agentd://metrics`/`agentd://capacity`); **P5** for
...
2. **Team kube-rs fluency (D2).** The Go verdict holds for any team WITHOUT deep
   kube-rs fluency (the common case). Does the team have it? If yes, and agentd
...
7. **Cross-repo sequencing (§14).** Are the agentd teams willing to take the
...
9. **A2A version (§7).** Which A2A version(s) does the gateway serve, and will
   agentd's `a2a` feature commit to those exact wire strings (true pass-through)
...
*This brainstorm consumes the agentd `rfcs/0014`–`0020` contracts and the
...
leak of cluster logic into agentd.*
